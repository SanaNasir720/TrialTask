...
public class PrefsManager : Singleton
{
   public void SaveGame() {
        ...
        for(int i = 0;i < arrayToSave.Length;i++) {
            GLPlayerPrefs.SetString("name", i.ToString(), arrayToSave[i].gameObject.name);
    ...
    public void LoadGame(CreationManager manager) {
        ...
        for (int i = 0; i < numberOfOjects; i++) {
            ...
            string name = GLPlayerPrefs.GetString("name", i.ToString());
            ...
            manager.CreateObjectWithAttributes(posVec, rot, scaleVec, name);
        }
